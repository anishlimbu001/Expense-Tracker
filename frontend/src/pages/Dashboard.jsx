...
import {ArrowDown, PiggyBank, Plus, TrendingDown, TrendingUp as ProfitIcon, Wallet, BarChart2, ChevronDown, ChevronUp, ShoppingCart, PieChart as PieChartIcon, DollarSign} from "lucide-react";
  ...
  return (
    <div className={dashboardStyles.container}>
      ...
      <div className={dashboardStyles.summaryGrid}>
        ...
        } label="Total Balance" value={`रु${Math.round(displayIncome - displayExpenses).toLocaleString()}`}
        additionalContent={
          <div className="flex items-center gap-2 mt-2 text-sm">
            <span className={dashboardStyles.balanceBadge}>
            +रु{Math.round(displayIncome).toLocaleString()}
            </span>
            <span className={dashboardStyles.expenseBadge}>
              -रु{Math.round(displayExpenses).toLocaleString()}
            </span>
            </div>
        }/>
        ...
        } label={`${timeFrameRange.label} Expenses`} value={`रु${Math.round(displayExpenses).toLocaleString()}`}
        additionalContent={
        ...
        } label={`${timeFrameRange.label} Savings`} value={`रु${Math.round(displaySavings).toLocaleString()}`}
        additionalContent={
        ...
        <div className={dashboardStyles.pieChartHeight}>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart className={chartStyles.pieChart}>
              ...
              <Tooltip
                formatter={(value) => [`रु${Math.round(value).toLocaleString()}`, "Amount"]}
              ...
              return (
                <div key={transaction.id} className={dashboardStyles.incomeTransactionItem}>
                  ...
                  <div className={dashboardStyles.transactionAmount}>
                    <p className={dashboardStyles.incomeAmount}>+रु{Math.abs(transaction.amount).toLocaleString()}</p>
            ...
            {displayedExpense.map((transaction) => {
              ...
              return (
                <div key={transaction.id} className={dashboardStyles.expenseTransactionItem}>
                  ...
                  <div className={dashboardStyles.transactionAmount}>
                    <p className={dashboardStyles.expenseAmount}>-रु{Math.abs(transaction.amount).toLocaleString()}</p>
                    <p className={dashboardStyles.transactionDate}>{new Date(transaction.date).toLocaleDateString()}</p>
                  </div>
                </div>
              );
