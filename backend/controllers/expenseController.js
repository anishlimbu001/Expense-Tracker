...
try{
    ...
    res.json({success: true, message: "Expense updated sucessfully.", data:
        updatedExpense
    });
}
...
export async function deleteExpense(req, res) {
     try{
        const income = await expenseModel.findByIdAndDelete({ _id: req.params.id});
        if(!income){
            return res.status(404).json({
                success: false,
                messagge: "Expense not found"
            });
        } 
        return res.json({
            success: true,
...
export async function downloadExpenseExcel(req, res) {
    ...
    try{
        ...
        const plainData = expense.map((exp)=> ({
            ...
            date: new Date(exp.date).toLocalDateString(),
        }));
